...
foo <- data.frame(label=c("Pelagic","Demersal","Total"),
                  mean=c(mean(p$y_Coefficient)*111,
                         mean(d$y_Coefficient)*111,
                         mean(t$y_Coefficient)*111),
                  sd=c(sd(p$y_Coefficient)*111,
                       sd(d$y_Coefficient)*111,
                       sd(t$y_Coefficient)*111))
par(mar=c(5,5,5,5))
x <- 1:3
plot(x,foo$mean,
     pch=19,
     xlab="",
     ylab="Latitudinal Range Shift (km/year)",
     xaxt="n",
     xlim=c(0.5,3.5),
     ylim=c(-10,10))
